...
namespace
{
    ...
    struct Instance
    {
        ...
        /* Stored row-wise pattern for value-only updates */
        std::vector<HYPRE_Int> stored_row_nnz;
        std::vector<HYPRE_Int> stored_row_idx;
        std::vector<HYPRE_BigInt> stored_cols;
        std::vector<mwSize> coo_to_rowmajor; /* maps COO input order -> row-major position */
        int omp_threads = 16;
    };
    ...
    void destroy_instance(Instance &inst)
    {
        ...
        if (inst.interp_ij)
        {
            for (int k = 0; k < inst.num_interp_vectors; k++)
            {
                if (inst.interp_ij[k])
                {
                    HYPRE_IJVectorDestroy(inst.interp_ij[k]);
                }
            }
    ...
    std::string to_id_string(const mxArray *id_arr)
    {
        if (mxIsChar(id_arr))
        {
            ...
            if (!s)
            {
                mex_error("hypre:badId", "Failed to convert char instance_id.");
            }
            std::string out(s);
            mxFree(s);
            return out;
    ...
    {
        if (!opts || mxIsEmpty(opts))
        {
            return nullptr;
        }
        if (!mxIsStruct(opts))
        {
            mex_error("hypre:badOpts", "opts must be a struct or empty.");
        }
    ...
    BoomerOptions parse_options(const mxArray *opts, mwSize n, mwSize null_k)
    {
        ...
        if (get_double_field(opts, "strong_threshold_r", o.strong_threshold_r))
        {
            o.has_strong_threshold_r = true;
        }
        ...
        if (get_double_field(opts, "interp_vec_abs_qtrunc", o.interp_vec_abs_qtrunc))
        {
            o.has_interp_vec_abs_qtrunc = true;
        }
        get_int_field(opts, "smooth_interp_vectors", o.smooth_interp_vectors);
        get_int_field(opts, "interp_refine", o.interp_refine);

        if (get_double_field(opts, "trunc_factor", o.trunc_factor))
        {
            o.has_trunc_factor = true;
        }
        if (get_int_field(opts, "agg_interp_type", o.agg_interp_type))
        {
            o.has_agg_interp_type = true;
        }
        if (get_double_field(opts, "agg_trunc_factor", o.agg_trunc_factor))
        {
            o.has_agg_trunc_factor = true;
        }
        if (get_int_field(opts, "agg_p_max_elmts", o.agg_p_max_elmts))
        {
            o.has_agg_p_max_elmts = true;
        }
        if (get_int_field(opts, "max_coarse_size", o.max_coarse_size))
        {
            o.has_max_coarse_size = true;
        }
        if (get_int_field(opts, "min_coarse_size", o.min_coarse_size))
        {
            o.has_min_coarse_size = true;
        }
        ...
        if (get_int_field(opts, "num_functions", o.num_functions))
        {
            o.has_num_functions = true;
        }
    ...
    void build_rowwise_from_coo(const double *I_data, const double *J_data,
                                const double *V_data, mwSize nnz_in, mwSize n,
                                std::vector<HYPRE_Int> &row_nnz,
                                std::vector<HYPRE_Int> &row_idx,
                                std::vector<HYPRE_BigInt> &rows,
                                std::vector<HYPRE_BigInt> &cols,
                                std::vector<HYPRE_Complex> &vals,
                                std::vector<mwSize> &coo_perm)
    {
        /* Count per-row nnz (input is 1-based) */
        std::vector<HYPRE_Int> row_ptr(n + 1, 0);
        for (mwSize k = 0; k < nnz_in; k++)
        {
            mwSize i = static_cast<mwSize>(std::llround(I_data[k])) - 1;
            row_ptr[i + 1]++;
        }
        for (mwSize i = 0; i < n; i++)
        {
        ...
        mwSize total_nnz = static_cast<mwSize>(row_ptr[n]);
        ...
        cols.resize(total_nnz);
        vals.resize(total_nnz);
        coo_perm.resize(nnz_in);
        ...
        for (mwSize k = 0; k < nnz_in; k++)
        {
            mwSize i = static_cast<mwSize>(std::llround(I_data[k])) - 1;
            HYPRE_Int dst = next[i]++;
            cols[dst] = static_cast<HYPRE_BigInt>(
                static_cast<mwSize>(std::llround(J_data[k])) - 1);
            vals[dst] = static_cast<HYPRE_Complex>(V_data[k]);
            coo_perm[k] = static_cast<mwSize>(dst);
        }
    }
                          ...
                          const mxArray *opts)
    {
        ...
        if (o.has_trunc_factor)
        {
            HYPRE_BoomerAMGSetTruncFactor(inst.solver, o.trunc_factor);
        }
        if (o.has_agg_interp_type)
        {
            HYPRE_BoomerAMGSetAggInterpType(inst.solver, o.agg_interp_type);
        }
        if (o.has_agg_trunc_factor)
        {
            HYPRE_BoomerAMGSetAggTruncFactor(inst.solver, o.agg_trunc_factor);
        }
        if (o.has_agg_p_max_elmts)
        {
            HYPRE_BoomerAMGSetAggPMaxElmts(inst.solver, o.agg_p_max_elmts);
        }
        if (o.has_max_coarse_size)
        {
            HYPRE_BoomerAMGSetMaxCoarseSize(inst.solver, o.max_coarse_size);
        }
        if (o.has_min_coarse_size)
        {
            HYPRE_BoomerAMGSetMinCoarseSize(inst.solver, o.min_coarse_size);
        }
    ...
    /* ================================================================
     *  setup_ijv:  create instance from COO triplets (1-based I,J)
     * ================================================================ */
    double setup_instance_ijv(Instance &inst,
                              const double *I_data, const double *J_data,
                              const double *V_data, mwSize nnz_in,
                              mwSize n,
                              const mxArray *Z,
                              const mxArray *opts)
    {
        ...
        {
            if (!mxIsDouble(Z) || mxIsComplex(Z))
                mex_error("hypre:badZ", "null space must be real double.");
            if (mxGetM(Z) != n)
                mex_error("hypre:badZ", "null space must have size n x k.");
        }
        ...
        /* Build row-wise arrays from COO and store permutation */
        std::vector<HYPRE_Int> row_nnz;
        std::vector<HYPRE_Int> row_idx;
        std::vector<HYPRE_BigInt> rows;
        std::vector<HYPRE_BigInt> cols;
        std::vector<HYPRE_Complex> vals;
        std::vector<mwSize> coo_perm;
        build_rowwise_from_coo(I_data, J_data, V_data, nnz_in, n,
                               row_nnz, row_idx, rows, cols, vals, coo_perm);
        inst.nnz = vals.size();
        inst.rows = rows;
        inst.stored_row_nnz = row_nnz;
        inst.stored_row_idx = row_idx;
        inst.stored_cols = cols;
        inst.coo_to_rowmajor = coo_perm;
    ...
    void command_setup_ijv(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
    {
        /* hypre_boomeramg_mex('setup_ijv', I, J, V, n, Z, opts, instance_id)
         *
         * If an instance with the given id already exists (created via
         * setup_ijv, i.e. has a stored COO permutation), the sparsity
         * pattern is assumed unchanged and only the values V are updated
         * (same fast path as update_values).  To force a full rebuild,
         * set opts.force_rebuild = true.
         */
        if (nrhs != 8)
            mex_error("hypre:usage", "setup_ijv usage: hypre_boomeramg_mex('setup_ijv', I, J, V, n, Z, opts, id)");

        const mxArray *mI = prhs[1];
        const mxArray *mJ = prhs[2];
        const mxArray *mV = prhs[3];
        mwSize n = static_cast<mwSize>(std::llround(mxGetScalar(prhs[4])));
        const mxArray *Z = prhs[5];
        const mxArray *opts = prhs[6];
        std::string id = to_id_string(prhs[7]);

        if (!mxIsDouble(mI) || !mxIsDouble(mJ) || !mxIsDouble(mV))
            mex_error("hypre:badIJV", "I, J, V must be real double vectors.");
        mwSize nnz_in = mxGetNumberOfElements(mV);
        if (mxGetNumberOfElements(mI) != nnz_in || mxGetNumberOfElements(mJ) != nnz_in)
            mex_error("hypre:badIJV", "I, J, V must have equal length.");

        /* --- Check for existing instance with reusable pattern --- */
        bool force_rebuild = false;
        if (opts && mxIsStruct(opts))
        {
            mxArray *fr = mxGetField(opts, 0, "force_rebuild");
            if (fr)
            {
                if (mxIsLogicalScalarTrue(fr))
                    force_rebuild = true;
                else if (mxIsDouble(fr) && mxGetScalar(fr) != 0.0)
                    force_rebuild = true;
            }
        }

        auto it = g_instances.find(id);
        if (it != g_instances.end() && !force_rebuild &&
            !it->second->coo_to_rowmajor.empty() &&
            nnz_in == it->second->coo_to_rowmajor.size())
        {
            /* ---- Fast path: reuse sparsity pattern, update values only ---- */
            Instance &inst = *it->second;

#ifdef _OPENMP
            if (inst.omp_threads > 0)
                omp_set_num_threads(inst.omp_threads);
#endif

            const double *v_in = mxGetPr(mV);
            mwSize nnz_rowmajor = inst.stored_cols.size();
            std::vector<HYPRE_Complex> vals_rm(nnz_rowmajor);
            for (mwSize k = 0; k < nnz_in; k++)
            {
                vals_rm[inst.coo_to_rowmajor[k]] = static_cast<HYPRE_Complex>(v_in[k]);
            }

            HYPRE_IJMatrixInitialize(inst.A_ij);
            HYPRE_IJMatrixSetValues2(inst.A_ij, static_cast<HYPRE_Int>(inst.n),
                                     inst.stored_row_nnz.data(), inst.rows.data(),
                                     inst.stored_row_idx.data(), inst.stored_cols.data(),
                                     vals_rm.data());
            HYPRE_IJMatrixAssemble(inst.A_ij);
            ...
            if (nlhs > 0)
            {
                const char *fields[] = {"instance_id", "n", "nnz", "num_functions",
                                        "num_interp_vectors", "setup_time_seconds"};
                mxArray *out = mxCreateStructMatrix(1, 1, 6, fields);
                mxSetField(out, 0, "instance_id", mxCreateString(id.c_str()));
                mxSetField(out, 0, "n", mxCreateDoubleScalar(static_cast<double>(inst.n)));
                mxSetField(out, 0, "nnz", mxCreateDoubleScalar(static_cast<double>(inst.nnz)));
                mxSetField(out, 0, "num_functions", mxCreateDoubleScalar(static_cast<double>(inst.num_functions)));
                mxSetField(out, 0, "num_interp_vectors", mxCreateDoubleScalar(static_cast<double>(inst.num_interp_vectors)));
                mxSetField(out, 0, "setup_time_seconds", mxCreateDoubleScalar(t_setup));
                plhs[0] = out;
            }
            return;
        ...
        auto inst = std::make_unique<Instance>();
        double t_setup = setup_instance_ijv(*inst, mxGetPr(mI), mxGetPr(mJ), mxGetPr(mV),
                                            nnz_in, n, Z, opts);
        g_instances[id] = std::move(inst);
    ...
    /* ================================================================
     *  update_values:  re-use existing pattern, update matrix values,
     *                  then re-run BoomerAMG setup.
     * ================================================================ */
    void command_update_values(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
    {
        /* hypre_boomeramg_mex('update_values', V, instance_id) */
        if (nrhs != 3)
            mex_error("hypre:usage", "update_values usage: hypre_boomeramg_mex('update_values', V, id)");

        const mxArray *mV = prhs[1];
        std::string id = to_id_string(prhs[2]);

        auto it = g_instances.find(id);
        if (it == g_instances.end())
            mex_error("hypre:noInstance", "Unknown instance_id. Call setup_ijv first.");
        Instance &inst = *it->second;

        if (!mxIsDouble(mV) || mxIsComplex(mV))
            mex_error("hypre:badV", "V must be a real double vector.");
        mwSize v_len = mxGetNumberOfElements(mV);
        if (inst.coo_to_rowmajor.empty())
            mex_error("hypre:noPattern", "Instance was not created via setup_ijv; cannot update values.");
        if (v_len != inst.coo_to_rowmajor.size())
            mex_error("hypre:badV", "V length must match original COO triplet length.");
        ...
        mwSize nnz_rowmajor = inst.stored_cols.size();
        std::vector<HYPRE_Complex> vals_rm(nnz_rowmajor);
        for (mwSize k = 0; k < v_len; k++)
        {
            vals_rm[inst.coo_to_rowmajor[k]] = static_cast<HYPRE_Complex>(v_in[k]);
        }

        /* Cheap re-init + set + assemble (pattern unchanged) */
        HYPRE_IJMatrixInitialize(inst.A_ij);
        HYPRE_IJMatrixSetValues2(inst.A_ij, static_cast<HYPRE_Int>(inst.n),
                                 inst.stored_row_nnz.data(), inst.rows.data(),
                                 inst.stored_row_idx.data(), inst.stored_cols.data(),
        ...
        /* Re-run AMG setup with new matrix values */
#ifdef _OPENMP
        double t0 = omp_get_wtime();
#else
        double t0 = 0.0;
        ...
        if (nlhs > 0)
        {
            plhs[0] = mxCreateDoubleScalar(t_setup);
        }
    }
...
extern "C" void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    try
    {
        ...
        if (cmd == "setup_ijv")
        {
            command_setup_ijv(nlhs, plhs, nrhs, prhs);
            return;
        }
        if (cmd == "update_values")
        {
            command_update_values(nlhs, plhs, nrhs, prhs);
            return;
        }
        if (cmd == "apply")
        {
