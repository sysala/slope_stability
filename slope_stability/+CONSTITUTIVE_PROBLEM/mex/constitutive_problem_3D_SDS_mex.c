/*
 * constitutive_problem_3D_SDS_mex.c
 *
 * Mex wrapper for stress + consistent-tangent evaluation of the 3D
 * Mohr-Coulomb constitutive model.  OpenMP-parallel over integration points.
 *
 * Usage from Octave / MATLAB:
 *   [S, DS] = constitutive_problem_3D_SDS_mex(E, c_bar, sin_phi,
 *                                              shear, bulk, lame)
 *
 ...
 *
 *   S        – 6  × n_int  (stress)
 *   DS       – 36 × n_int  (consistent tangent, column-major 6×6 blocks)
 */
...
{
    if (nrhs != 6)
        mexErrMsgTxt("constitutive_problem_3D_SDS_mex: "
                     "6 inputs required (E, c_bar, sin_phi, shear, bulk, lame)");
    if (nlhs > 2)
        mexErrMsgTxt("constitutive_problem_3D_SDS_mex: up to 2 outputs (S, DS)");
    ...
    plhs[1] = mxCreateDoubleMatrix(36, n_int, mxREAL);
